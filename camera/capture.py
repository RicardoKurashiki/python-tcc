...
def capture_images(letra, nome):
    ...
    while True:
        ...
        cv2.putText(frame, folder + ": "+str(img_counter), (30, 400),
        ...
        if cv2.waitKey(1) == CAPTURE:
            ...
            if t_counter > QTD_TRAIN and t_counter <= (QTD_TRAIN+QTD_TEST):
                img_name = dir_img_test + \
                    str(letra) + "/"+nome+"{}.png".format(test_set_image_name)
                save_img = cv2.resize(result, (image_x, image_y))
                cv2.imwrite(img_name, save_img)
                print("{} written!".format(img_name))
                test_set_image_name += 1
...
letra = input("LETRA: ")
nome = input("NOME: ")
create_path(dir_img_training)
create_path(dir_img_test)
